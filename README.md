...
The site currently features 39 tools organized around the hierarchy used by the universal menu and All Tools directory:
...
- Word Games: [Word Unscrambler](https://monkeytactics.com/tools/word-unscrambler), [Anagram Architect](https://monkeytactics.com/tools/anagram-architect), [Anagram Animator](https://monkeytactics.com/tools/anagram-animator), [Words With Friends Solver](https://monkeytactics.com/tools/words-with-friends-solver), [Crossword Clue & Pattern Solver](https://monkeytactics.com/tools/crossword-solver), [Wordiply Solver](https://monkeytactics.com/tools/wordiply-solver), [Wordle Solver & Helper](https://monkeytactics.com/tools/wordle-helper), [Antiwordle Solver](https://monkeytactics.com/tools/antiwordle-solver), [Absurdle Solver](https://monkeytactics.com/tools/absurdle-solver), [Quordle Solver](https://monkeytactics.com/tools/quordle-solver), [Octordle Solver](https://monkeytactics.com/tools/octordle-solver), [Sedecordle Solver](https://monkeytactics.com/tools/sedecordle-solver)
...
|---|---|---|
| **Word Games** | — | Word Unscrambler, Anagram Architect, Anagram Animator, Words With Friends Solver, Crossword Solver, Wordiply Solver, Wordle Solver, Antiwordle Solver, Absurdle Solver, Quordle Solver, Octordle Solver, Sedecordle Solver |
...
MonkeyTactics is primarily a static site built from lightweight HTML, CSS, and JavaScript, with a narrowly scoped Cloudflare Pages Function for dynamic Word Unscrambler routes. Nine first-party Rust components compile to WebAssembly. Every generated browser package is committed under `assets/wasm/`, so production hosting does not require a Rust toolchain.

The Word Unscrambler, Anagram Architect, Words With Friends Solver, Crossword Solver, Wordiply Solver, Wordle Solver, Antiwordle Solver, Absurdle Solver, Quordle Solver, Octordle Solver, and Sedecordle Solver use two local dictionaries: Standard, built from the public-domain ENABLE list, and Expanded, built from Wiktionary. Their combined 867,177-word union is split into 26 versioned gzip chunks that the browser decompresses and indexes locally. Anagram Architect uses a dedicated Rust/WebAssembly search engine, multiple workers, local language metadata, and phrase-ranking data to generate exact multi-word anagrams without sending the source phrase to a server. Its Pick List stores versioned discovery recipes that can be copied, validated, imported, restored, and rerun. The standalone Anagram Animator validates before-and-after text and renders compact focus-mode or embedded reveals locally from URL parameters instead of uploading video. The Crossword Solver also lazy-loads a compressed index of 129,492 automatically filtered WordNet 3.0 clue records, including 34,636 multi-word answers. The Wordle-family solvers apply duplicate-aware green, yellow, and gray feedback; Absurdle additionally scores adversarial feedback partitions to minimize the largest answer pool the game can preserve. All matching remains local to the browser.
...
| [Anagram Architect](https://monkeytactics.com/tools/anagram-architect) | `wasm/anagram-architect-engine` | `assets/wasm/anagram-architect` | Parallel exact-phrase generation, letter accounting, constraints, pruning, and ranked candidate retention |
| [Words With Friends Solver](https://monkeytactics.com/tools/words-with-friends-solver) | `wasm/word-unscrambler-engine` | `assets/wasm/word-unscrambler` | Word Unscrambler workflow with Words With Friends tile values and 35-point bingo rules |
| [Crossword Solver](https://monkeytactics.com/tools/crossword-solver) | `wasm/word-unscrambler-engine` | `assets/wasm/word-unscrambler` | WASM pattern search, WordNet graph ranking, multi-word answers, explainable scores, grouped candidate picks, JSON import/export, compact share links, and QR codes |
...
| [Antiwordle Solver](https://monkeytactics.com/tools/antiwordle-solver) | `wasm/word-unscrambler-engine` | `assets/wasm/word-unscrambler` | Candidate filtering plus browser-side mandatory-rule validation and avoidance-focused safe-guess ranking |
| [Absurdle Solver](https://monkeytactics.com/tools/absurdle-solver) | `wasm/word-unscrambler-engine` | `assets/wasm/word-unscrambler` | Candidate filtering plus browser-side adversarial partition scoring and minimax next-guess ranking |
...
| [Word & Character Counter](https://monkeytactics.com/tools/word-character-counter) | `wasm/text-analyzer-engine` | `assets/wasm/text-analyzer` | Worker-based multilingual metrics, readability, keyword and structure analysis, IndexedDB autosave, and a complete JavaScript fallback |
...
All seven components run locally in the browser. Their Rust source lives under `wasm/`; generated JavaScript bindings and `.wasm` binaries live under `assets/wasm/`.
...
The Crossword Solver uses the same bridge with its dedicated interface:
...
The Antiwordle and Absurdle solvers reuse that bridge with dedicated browser-side strategy layers. Antiwordle enforces mandatory clue rules and ranks guesses for avoidance; Absurdle partitions every remaining answer by its possible feedback and ranks guesses by the smallest worst-case partition.

Dictionary shards remain lazy-loaded by both interfaces and are passed to `init_engine` as arrays of `word\\tmembership` records. The bridge exposes rack-constrained `unscramble` and pattern-first `crosswordSearch` operations. Crossword clue searches separately load a compressed keyword-and-synonym index and length-based WordNet data shards from `assets/data/crossword-clues/`; clue ranking runs in JavaScript and combines literal relevance, discounted synonym expansion, pattern, length, and dictionary membership. Its browser-local Pick List stores clue context, definitions, match details, grid positions, and notes; saved picks can restore searches or generate compact URLs, formatted clipboard summaries, and QR codes. The WASM module authorizes `monkeytactics.com`, the Cloudflare Pages production and preview hosts under `monkeytactics-calculators.pages.dev`, and the local Wrangler host `127.0.0.1`.
...
2.0 used by Tesseract.js and the WordNet 3.0 license. See [Third-Party Notices](THIRD_PARTY_NOTICES.md) for sources,
