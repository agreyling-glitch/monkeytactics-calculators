...
- Word Games: [Word Unscrambler](https://monkeytactics.com/tools/word-unscrambler), [Words With Friends Solver](https://monkeytactics.com/tools/words-with-friends-solver), [Crossword Clue & Pattern Solver](https://monkeytactics.com/tools/crossword-solver), [Wordle Solver & Helper](https://monkeytactics.com/tools/wordle-helper), [Antiwordle Helper](https://monkeytactics.com/tools/antiwordle-helper), [Quordle Solver](https://monkeytactics.com/tools/quordle-solver), [Octordle Solver](https://monkeytactics.com/tools/octordle-solver), [Sedecordle Solver](https://monkeytactics.com/tools/sedecordle-solver)
...
|---|---|---|
| **Word Games** | — | Word Unscrambler, Words With Friends Solver, Crossword Solver, Wordle Solver, Antiwordle Helper, Quordle Solver, Octordle Solver, Sedecordle Solver |
...
| [Crossword Solver](https://monkeytactics.com/tools/crossword-solver) | `wasm/word-unscrambler-engine` | `assets/wasm/word-unscrambler` | WASM pattern-first dictionary search plus browser-side WordNet clue ranking, known-letter matching, answer length, and optional filters |
...
| [Antiwordle Helper](https://monkeytactics.com/tools/antiwordle-helper) | `wasm/word-unscrambler-engine` | `assets/wasm/word-unscrambler` | Candidate filtering plus browser-side mandatory-rule validation and avoidance-focused safe-guess ranking |
...
Dictionary shards remain lazy-loaded by both interfaces and are passed to `init_engine` as arrays of `word\\tmembership` records. The bridge exposes rack-constrained `unscramble` and pattern-first `crosswordSearch` operations. Crossword clue searches separately load a compressed keyword index and length-based WordNet data shards from `assets/data/crossword-clues/`; clue ranking runs in JavaScript and combines meaning relevance with pattern, length, and dictionary membership. The WASM module authorizes `monkeytactics.com`, the Cloudflare Pages production and preview hosts under `monkeytactics-calculators.pages.dev`, and the local Wrangler host `127.0.0.1`.
