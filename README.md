...
|---|---|---|
| **Word Games** | — | Word Unscrambler, Words With Friends Solver, Crossword Solver, Wordiply Solver, Wordle Solver, Antiwordle Solver, Absurdle Solver, Quordle Solver, Octordle Solver, Sedecordle Solver |
...
The All Tools page mirrors this hierarchy. The same structure is rendered by the Rust/WASM hamburger menu on every page, while global search supports pointer and keyboard selection. The homepage features Crossword Solver, Compound Interest Calculator, Loan & Mortgage Calculator, Wordle Solver, Antiwordle Solver, Absurdle Solver, Word Unscrambler, Words With Friends Solver, Quordle Solver, Octordle Solver, and Sedecordle Solver.
...
The Word Unscrambler, Words With Friends Solver, Crossword Solver, Wordiply Solver, Wordle Solver, Antiwordle Solver, Absurdle Solver, Quordle Solver, Octordle Solver, and Sedecordle Solver share two selectable dictionaries: Standard, built from the public-domain ENABLE list, and Expanded, built from Wiktionary. Both combines their memberships without duplicate results. The 867,177-word union is split into 26 versioned gzip chunks with a small manifest, and the browser decompresses and indexes words locally. The Crossword Solver also lazy-loads a compressed index of 129,492 automatically filtered WordNet 3.0 clue records, including 34,636 multi-word answers. JavaScript ranks literal clue matches first, adds discounted same-synset matches and weighted WordNet graph relationships, then combines them with answer-pattern constraints; the existing Rust/WASM engine continues to handle pattern-only dictionary searches. Phrase answers preserve spaces for display while matching grid patterns by letters only. The Wordle-family solvers apply duplicate-aware green, yellow, and gray feedback; Absurdle additionally scores adversarial feedback partitions to minimize the largest answer pool the game can preserve. All matching remains local to the browser.
